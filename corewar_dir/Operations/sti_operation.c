/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   sti_operation.c                                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ayavorsk <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/04/24 15:33:53 by ayavorsk          #+#    #+#             */
/*   Updated: 2018/04/26 12:40:14 by jdoeorsk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/operations.h"

int				check_register(t_vm *vm, t_process *cur)
{
	if ((if_good_reg_start(vm, cur)) == 0)
		return (0);
	if ((IS_REG_M(vm->memory[overla(cur->pc + 1)])))
	{
		if ((if_good_reg_midl(vm, cur, 3)) == 0)
			return (0);
		if ((if_good_reg_end(vm, cur, 4)) == 0)
			return (0);
	}
	else if ((if_good_reg_end(vm, cur, 5)) == 0)
		return (0);
	return (1);
}

static void		reg_dir(t_vm *vm, t_process *cur, int pos)
{
	int		f2;
	int		f3;
	int		shift;
	int		i;

	i = 0;
	shift = 2;
	f2 = first_operand_2(vm, cur, &shift);
	f3 = second_operand_2(vm, cur, &shift);
	vm->res = ((int)f2 + f3) % IDX_MOD;
	if (check_register(vm, cur))
	{
		while (i < REG_SIZE)
		{
			vm->memory[overla(cur->pc + get_idx_mod(vm->res) + i)] = \
			cur->reg[pos - 1][i];
			vm->memory_color[(cur->pc + get_idx_mod(vm->res)) % \
				MEM_SIZE + i] = cur->color;
			i++;
		}
	}
}

static void		reg_ind(t_vm *vm, t_process *cur, int pos)
{
	int		f2;
	int		f3;
	int		shift;
	int		i;

	i = 0;
	shift = 5;
	f2 = (get_4_bytes(vm, (get_idx_mod(
		get_2_bytes(vm, overla(cur->pc + 3))) + cur->pc) % MEM_SIZE));
	f3 = (second_operand_2(vm, cur, &shift));
	vm->res = ((int)f2 + (int)f3) % IDX_MOD;
	if (check_register(vm, cur))
	{
		while (i < REG_SIZE)
		{
			vm->memory[overla(cur->pc + get_idx_mod(vm->res) + i)] = \
			cur->reg[pos - 1][i];
			vm->memory_color[overla(cur->pc + get_idx_mod(vm->res) + i)] = \
			cur->color;
			i++;
		}
	}
}

void			sti_operation(t_vm *vm, t_process *cur)
{
	int pos;

	pos = vm->memory[(cur->pc + 2) % MEM_SIZE];
	cur->value = vm->memory[overla(cur->pc + 1)];
	if (check_c_octal_sti(vm->memory[overla(cur->pc + 1)]))
	{
		if (IS_REG_S(vm->memory[overla(cur->pc + 1)])
		&& !IS_IND_M(vm->memory[overla(cur->pc + 1)]))
			reg_dir(vm, cur, pos);
		else if (IS_REG_S(vm->memory[overla(cur->pc + 1)])
		&& IS_IND_M(vm->memory[overla(cur->pc + 1)])
		&& (IS_REG_E(vm->memory[overla(cur->pc + 1)])
			|| IS_DIR_E(vm->memory[overla(cur->pc + 1)])))
		{
			reg_ind(vm, cur, pos);
		}
	}
	move_pc_st(cur, vm, 2, 3);
}
