...
function check_1()
{
  echo "\034[1;32m [2 - players test]\033[0m "$END" times"
  for ((i=0; i<$END ; i++));
    do
     bot1=${bot[$(((RANDOM*50%$BOTCNT)))]}
     perl -e 'alarm shift @ARGV; exec @ARGV' 30 ./corewar ${bot1} -d $DUMP > f1
      ./original/corewar ${bot1} -d $DUMP > f2
      DIFF=$(diff f1 f2) 
     if ["$DIFF" == ""];
       then
  ...
  for ((i=0; i<$END ; i++));
    do
     ...
     bot2=${bot[$(((RANDOM*37%$BOTCNT)))]}
      perl -e 'alarm shift @ARGV; exec @ARGV' 30 ./corewar  ${bot1} ${bot2} -d $DUMP > f1
      ./original/corewar ${bot1} ${bot2} -d $DUMP > f2
      DIFF=$(diff f1 f2) 
     if ["$DIFF" == ""];
       then
...
function check_3()
{
  echo "\034[1;32m [3 - players test]\033[0m "$END" times"
  for ((i=0; i<$END; i++));
  do
   bot1=${bot[$(((RANDOM*77%$BOTCNT)))]}
   bot2=${bot[$(((RANDOM*37%$BOTCNT)))]}
   bot3=${bot[$(((RANDOM*22%$BOTCNT)))]}
   perl -e 'alarm shift @ARGV; exec @ARGV' 120 ./corewar  ${bot1} ${bot2} ${bot3} -d $DUMP > f1
    ./original/corewar ${bot1} ${bot2} ${bot3} -d $DUMP > f2
    DIFF=$(diff f1 f2 ) 
   if ["$DIFF" == ""]; then
...
function check_4()
{
  ...
  do
   ...
   perl -e 'alarm shift @ARGV; exec @ARGV' 180 ./corewar  ${bot1} ${bot2} ${bot3} ${bot4} -d $DUMP > f1
    ./original/corewar ${bot1} ${bot2} ${bot3} ${bot4} -d $DUMP > f2
    DIFF=$(diff f1 f2) 
   if ["$DIFF" == ""]; then
