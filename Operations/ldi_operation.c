...
void	ldi_operation(t_VM *vm, t_process *cur)
{
    ...
    {
            shift = 3;
            f1 = get_idx_mod(get_4_bytes(vm, get_idx_mod(get_2_bytes(vm, overla(cur->pc + 2)))));
            f2 = second_operand_3(vm, cur, &shift);
            if (check_register(vm, cur))
                load_to_reg(cur, (int) vm->memory[overla(cur->pc + shift + 1)] - 1, overla(get_idx_mod(f1 + f2) + cur->pc));
    }
    move_pc(cur, vm, 2, 3);
}